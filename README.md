...
| **External Engine Viewport** | **Bevy host-mode zero-copy viewport (v0.15.0); Godot 4 async readback + experimental GPU blit (v0.15.0 — true zero-copy needs upstream Godot patches)** | `PaintCallback` + `bevy_egui` | `widget::shader` | `Image::try_from(wgpu::Texture)` | Canvas / fork |
...
Martensite's automated accessibility rule engine (`martensite-access`)
checks WCAG 2.2 AA/AAA contrast, target size, and focus appearance for
the standard widget palette. However, full Section 508 / VPAT
conformance requires **manual testing with assistive technology** that
cannot be automated in CI:

- **NVDA (Windows)**: Install NVDA and verify caret navigation, live
  region announcements, and screen-reader focus tracking.
- **VoiceOver (macOS)**: Enable VoiceOver and verify the same.
- **Orca (Linux)**: Install Orca and verify via AT-SPI2.

If you are an accessibility specialist and can run manual AT testing,
see `WORKING_ON.md` section 12.4 for the criteria that require manual
evaluation. The automated `VpatReport` generated by
`martensite-access::compliance` is an evaluation aid, not a
certification.
