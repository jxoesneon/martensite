...
pub struct SourceJournal {
    /// All recorded writes in monotonic index order.
...
impl SourceJournal {
    ...
    /// Returns all recorded writes in monotonic index order.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// Returns the write at `index`, or `None` if out of range.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// Drops all recorded writes. The index counter restarts at 0.
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// recording is suppressed by a [`JournalGuard`].
    pub(crate) fn record(
        &mut self,
        signal: SignalId,
        previous: Box<dyn Any + Send + Sync>,
...
impl std::fmt::Debug for SourceJournal {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SourceJournal")
