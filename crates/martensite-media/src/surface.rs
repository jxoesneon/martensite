...
// the handle/error/format/metadata types for its decoder output).
// See `martensite-media-platform::surface` for the canonical definitions.
...
impl VideoSurface {
    ...
    /// Returns an exclusive reference to the video frame metadata.
    ///
    /// Used by the decode pipeline to re-dimension the surface when a
    /// decoded frame's coded size differs from the placeholder.
    ///
    /// # Examples
    ///
    /// ```
    /// use martensite_media::surface::{VideoPixelFormat, VideoSurface};
    ///
    /// let mut surface = VideoSurface::new_mock(2, 2, VideoPixelFormat::Nv12);
    /// surface.metadata_mut().width = 1920;
    /// assert_eq!(surface.width(), 1920);
    /// ```
    #[inline]
    pub fn metadata_mut(&mut self) -> &mut VideoFrameMetadata {
        &mut self.metadata
    }
