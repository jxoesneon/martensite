...
//!
//! The paint pass produces an ordered list of [`PaintCommand`]s which are then
//! consumed by a render backend (e.g. `martensite-render`'s TinySkia or Vello
//! implementations). The representation is intentionally allocation-light: a
//! single [`PaintList`] can be cleared and reused across frames so that no
//! per-frame heap traffic is required for steady-state rendering.
...
///
/// ```
/// use martensite_core::GradientStop;
///
...
///
/// ```
/// use martensite_core::{GradientStop, GradientStops};
///
...
impl GradientStops {
    ...
    ///
    /// ```
    /// use martensite_core::GradientStops;
    ///
...
///
/// ```
/// use martensite_core::FontResource;
///
...
///
/// ```
/// use martensite_core::GlyphInstance;
///
...
impl GlyphRun {
    ...
    ///
    /// ```
    /// use martensite_core::{FontResource, GlyphRun};
    ///
    /// let font = FontResource::new(b"font bytes".to_vec(), 0);
...
///
/// ```
/// use martensite_core::{GlyphRun, PaintCommand, PaintList};
/// use kurbo::Rect;
///
...
///
/// ```
/// use martensite_core::{PaintList, PaintSegment};
/// use kurbo::Rect;
///
/// let mut list = PaintList::new();
...
///
/// ```
/// use martensite_core::PathBuilder;
/// use kurbo::Point;
///
...
impl PaintList {
    ...
    ///
    /// ```
    /// use martensite_core::{PaintCommand, PaintList, PathBuilder};
    /// use kurbo::Point;
    ///
    /// let mut builder = PathBuilder::new();
    /// builder.move_to(Point::new(0.0, 0.0));
    ...
    /// ));
    /// ```
    pub fn push_external(&mut self, surface_id: crate::SurfaceId, rect: [f32; 4], clip: [f32; 4]) {
...
mod tests {
    ...
    fn segments_omit_empty_spans() {
        ...
        let mut list = PaintList::new();
        list.push_external(crate::SurfaceId(1), [0.0; 4], [0.0; 4]);
        list.push_fill_rect(Rect::ZERO, [0, 0, 0, 255]);
        list.push_external(crate::SurfaceId(2), [0.0; 4], [0.0; 4]);
        let segments = list.segments();
        assert_eq!(segments.len(), 3);
        assert!(matches!(
            segments[0],
            PaintSegment::External {
                surface_id: crate::SurfaceId(1),
                ..
            }
        ));
