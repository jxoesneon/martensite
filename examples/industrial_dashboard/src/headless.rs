//! Industrial Workstation — the headless composition mode.
//!
//! The v0.18.0 CI dogfood, preserved: reactive signals driving state,
//! theme tokens, an arena of real widgets resolved by the Taffy layout
//! engine, a BSP docking tree, a virtualized 1,000,000-row `DataTable`,
//! chart series, a code editor model, a `MediaView` display surface
//! (stub surface — no video file required), keyboard focus navigation,
//! shell chrome material resolution, and a feature-gated devtools HUD.
//!
//! Runs without a display server — every subsystem is exercised through
//! its headless model APIs so `cargo run -p industrial_dashboard
//! --headless` works in CI. `F1`–`F16` comments mark the API friction
//! found while wiring the demo — pre-freeze fix candidates, see
//! docs/milestones/v0.18.0 §4.6. The windowed mode (default) lives in
//! `app.rs`; shared model types live in `model.rs`.
//!
//! Run headless with the devtools HUD enabled:
//! `cargo run -p industrial_dashboard --features devtools -- --headless`

use crate::model::{gen_rows, MetricRow};
