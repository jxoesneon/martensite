...
* **Technical Domain:** `software-cpu-fallback`

## Context and Problem Statement

For a sovereign, retained-mode, GPU-accelerated GUI engine targeting v1.0.0, handling software-cpu-fallback is a critical requirement. We must establish a robust, performant, and safe architecture.
...
* **Option 3**: **Fallback to tiny-skia for CPU rasterization when GPU compute (Vello) is unavailable**.

## Decision Outcome

Chosen option: **Option 3**. Fallback to tiny-skia for CPU rasterization when GPU compute (Vello) is unavailable perfectly aligns with the Anti-Slop Doctrine, providing explicit, measurable, and high-performance guarantees.
