# [ADR-0029] Plugin and Extension Security

* **Status:** Accepted
* **Date:** 2026-09-06
* **Deciders:** Master (Sovereign Architect), Ciel (Systems Architecture Guild)
* **Technical Domain:** `plugin-extension-security-model`

## Context and Problem Statement

For a sovereign, retained-mode, GPU-accelerated GUI engine targeting v1.0.0, handling plugin-extension-security-model is a critical requirement. We must establish a robust, performant, and safe architecture.
...
* **Option 3**: **Run plugins in WebAssembly (Wasmtime) sandboxes with explicit capability grants**.

## Decision Outcome

Chosen option: **Option 3**. Run plugins in WebAssembly (Wasmtime) sandboxes with explicit capability grants perfectly aligns with the Anti-Slop Doctrine, providing explicit, measurable, and high-performance guarantees.
