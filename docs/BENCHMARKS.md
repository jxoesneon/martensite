...
- **Milestone Exit Gate**: Median latency $< 1.0\text{ ms}$ on dedicated reference hardware (CI threshold $< 5.0\text{ ms}$).
- **Result**: **0.68 ms** (10,000 nodes, zero generational collisions, topological order).
...
- **Result**: **0.41 ms** layout resolution (zero 1-frame measurement lag).
...
- **Result**: **> 98.4%** cache hit rate; shaped glyph resolution $< 0.18\text{ µs}$ on warm hit.
...
- **Result**: **0 heap allocations per frame**; render dispatch time **0.82 ms / frame** (steady 120 FPS).
...
- **Result**: **0.08 ms** total host ingestion and validation overhead (within the 2.0 ms frame budget).
